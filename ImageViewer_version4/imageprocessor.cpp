...
void ImageProcessor::normalizeKernal(vector<double>& data){
    double sum = 0;
...
QImage ImageProcessor::GaussTransform(const QImage& img,int l,double s){
    QImage ret(img);
    vector<double> data;
    for(int i=-l/2;i<l/2+1;i++){
        for(int j=-l/2;j<l/2+1;j++){
//            qDebug()<<i<<" "<<j<<endl;
            double num = exp(-((i*i+j*j)/(2*s*s)));
            data.push_back(num);
        }
    }
//    qDebug()<<data;
    normalizeKernal(data);
    ret = linearFilter(ret,data,l,0,1);
    return ret;
}
QImage ImageProcessor::BilateralTransform(const QImage& img,int l,double s,double s1){
    QImage ret(img);
    vector<double> data;
    map<int,double> datalist;
    for(int i=-l/2;i<l/2+1;i++){
        for(int j=-l/2;j<l/2+1;j++){
            double num = exp(-((i*i+j*j)/(2*s*s)));
            data.push_back(num);
    ...
    for(int i=0;i<256;i++){
        datalist.insert(map<int,double>::value_type(i,exp(-(i*i)/(2*s1*s1))));
    }
    ...
    for(int i=l/2;i<w+l/2;i++){
        for(int j=l/2;j<h+l/2;j++){
            ...
            for(int k=0;k<data.size();k++){
                newdata.push_back(data[k]*datalist[abs(qGray(patch.pixel(k/l,k%l))-qGray(tempimg.pixel(i,j)))]);
            }
            normalizeKernal(newdata);
            for(int k=0;k<data.size();k++){

                sum+=newdata[k]*qGray(patch.pixel(k/l,k%l));
            }
            if(sum<0) sum=0;
            if(sum>255) sum=255;
